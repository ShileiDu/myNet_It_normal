...
class NormalizeUnitSphere(object):
    ...
    def normalize(pcl, center=None, scale=None):
        """
        Args:
            pcl:  The point cloud to be normalized, (N, 3), 2维tensor
        """
        if center is None:
            # pcl.max(dim=0, keepdim=True)[0]，2维tensor，1行3列，每一列保存pcl每一列的最大值
        ...
        if scale is None:
            ...
            # scale：将点云的中心移到原点后，找点云距离中心的最大欧氏距离
            scale = (pcl ** 2).sum(dim=1, keepdim=True).sqrt().max(dim=0, keepdim=True)[0]  # 2维tensor，(1, 1)
        pcl = pcl / scale
...
class AddLaplacianNoise(object):
    ...
    def __call__(self, data):
        ...
        data['noise_std'] = noise_std.float()
        return data
